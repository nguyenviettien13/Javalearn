...
public class MTDT {
    ...
    public void stateModel() {
        ...
        for (int i = 0; i < n; i++) {
            x_p[i][0] = new VarIntLS(ls, m1, m1 + m2 - 1);  // phản biện 1
            x_p[i][1] = new VarIntLS(ls, 0, m1 - 1);        // phản biện 2
            x_p[i][2] = new VarIntLS(ls, 0, m1 - 1);        // chủ tịch
            x_p[i][3] = new VarIntLS(ls, 0, m1 - 1);        // thư kí
            x_p[i][4] = new VarIntLS(ls, m1, m1 + m2 - 1);  // ủy viên
        }
        ...
        // Số lần xhien với vtro phản biện <= lambda
        VarIntLS[] y = copyColumn(x_p, 1);
        for (int i = 0; i < m1; i++) {
            // Đếm số lần gv i đóng vai trò phản biện 2
        ...
        for (int i = m1; i < m1 + m2; i++) {
            // Đếm số lần gv i đóng vai trò phản biện 1
            IFunction o = new Occurrence(y, i);
            S.post(new LessOrEqual(o, lambda));
        }
        ...
        for (int i = 0; i < m1 + m2; i++) {
            // Đếm số lần gv i xuất hiện trong các hội đồng
        ...
        for (int i = 0; i < n; i++) {
            // Element Function giúp tra cứu giá trị của 1 phần tử trong mảng match khi index là biến VarIntLS thay đổi
            match1[i] = new Element(match, i, x_p[i][0]);   // return match của luận văn i và giảng viên phản biện 1 hội đồng đó
            match2[i] = new Element(match, i, x_p[i][1]);   // return match của luận văn i và giảng viên phản biện 2 hội đồng đó
        }
    ...
    public void solve() {
        ...
        // search tim ra sol co vio = 0
        ts.search(S, 20, 300, 1000000, 200);

        // tabu search nhưng duy trì violation trong S = 0
        ts.searchMaintainConstraintsMinimize(obj1, S, 20, 30, 100000, 200);
        IFunction[] tf = new IFunction[1];
        tf[0] = obj1;

        // tabu search nhưng duy trì (violation trong S = 0 và giá trị tối ưu của hàm tf) nhưng tìm min của obj2
