...
public class BinPacking2D {
    ...
    public void stateModel() {
        ...
        for (int i = 0; i < n; i++) {
            // Khối hàng nằm trọn trong container
        ...
        for (int i = 0; i < n - 1; i++) {
            for (int j = i + 1; j < n; j++) {
                ...
                S.post(new Implicate(
                        new AND(new IsEqual(o[i], 0), new IsEqual(o[j], 1)),
                        new OR(c)));
                ...
                c[3] = new LessOrEqual(new FuncPlus(y[j], h[j]), y[i]); // l2.y<r1.y
                S.post(new Implicate(
                        new AND(new IsEqual(o[i], 1), new IsEqual(o[j], 0)),
                        new OR(c)));
                ...
                c[3] = new LessOrEqual(new FuncPlus(y[j], w[j]), y[i]); // l2.y<r1.y
                S.post(new Implicate(
                        new AND(new IsEqual(o[i], 1), new IsEqual(o[j], 1)),
                        new OR(c)));
            }
        }
        ls.close();
    ...
    public static void main(String[] args) {
        ...
        bp.readData("src\\data\\BinPacking2D\\bin-packing-2D-W19-H19-I21.txt");
        bp.solve(1000);
